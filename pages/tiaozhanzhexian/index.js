...
Page({
    ...
    jumpToTiaoZhan:function(e){
        ...
        wx.request({
            ...
            success: function (res) {
                ...
                if (fuxiList.length < 1) {
                    wx.showToast({
                        title: '去开始背单词中添加词进入掌握区吧',
                        icon: "none"
                    })
                    setTimeout(function(){
                        wx.switchTab({
     ...
     */
    onShow: function () {
        var staticData = [10, 20, 30, 40, 50, 60, 70, 80, 90];
        //获取用户的挑战数据
        var userID = wx.getStorageSync("userID");
        var that = this;
        wx.request({
            url: url.getTiaoZhanList(),
            data: { userID: userID },
            success: function (data) {
                ...
                if (countList.length > 0) {
                    ...
                    for (var i = 0; i < countList.length; i++) {
                        list[i+1] = parseFloat(countList[i]['rightscore']);
                    }
                    var dianList = list;
                    ...
                    for (var i = 0; i < staticData.length; i++) {
                        if (duan < staticData[i]) {
                            fianlDuan = parseInt(staticData[i]);
                            break;
                        }
                    }
                    ...
                    that.setData({
                        yOneDuan: fianlDuan,
                        heightLineNum: Math.ceil(max / fianlDuan) + 1,
